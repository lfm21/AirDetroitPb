...
```{r info about gathered dataset}
dim(Pb1981to2018.gathered) #[1] 5418 rows of 7 columns, 37 years of data, sample taken every 2.49 days
...
Mystery.site <- subset(Pb1981to2018.gathered, Site.ID = 261630020, select = c("1981-01-01":"1991-12-31"), na.rm = TRUE)
...
```{r schools.parks facet wrap}
summary(Pb1981to2018.points$Site.Name)

Pb.schools.parks <-  filter(Pb1981to2018.points, 
                      Site.Name %in% c("Southwestern H.S.",
                                       "PROPERTY OWNED BY DEARBORN PUBLIC SCHOOLS",
                                       "Allen Park", "Eliza Downwind", 
...
Pb.schools.parks.facet <- ggplot(Pb.schools.parks) +
  geom_point(aes(x = Site.Name, y = Daily.Mean.Pb.Concentration, fill = Date),
             size = 0.5, alpha = 0.5) +
  facet_grid(rows = vars(Site.Name),
             scales = "free_y", ncol = 1, strip.position = "right") +
  #scale_y_log10() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8), 
        legend.position = "bottom") +
  labs(x = "Site", y = "Air Lead Level, ug/m3", title = )
  print(Pb.schools.plot)
```
