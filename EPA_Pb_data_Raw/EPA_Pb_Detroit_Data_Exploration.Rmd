...
```{r attempt at function for reading csv}
get <- Pb1981
...
Detroitmap.1 <- st_read("./Master_Plan_Neighborhoods_Detroit/Master_Plan_Neighborhoods_Detroit.shp")
st_crs(Detroitmap.1)
plot(Detroitmap.1)
...
Pb.schools.parks <-  Pb1981to2018.points %>%
  ...
  Pb.schools.parks.plot <- ggplot(schools.parks, 
                           aes(x = year, y = Daily.Mean.Pb.Concentration,
                               color = year)) +
  geom_point(aes(), size = 0.5, alpha = 0.5) +
  facet_wrap(vars(Site.Name), scales = "free_y", ncol = 1, strip.position = "right") +
...
```{r schools.parks adding month and year}
library("lubridate") 
...
```{r - facet NOT WORKING for facet-wrap and y scale}
