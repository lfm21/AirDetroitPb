...
```{r MAYBE DONT NEED THIS look at the .shp }
#Detroit.neighborhoods <- 
  st_read
("./EPA_Pb_data_Raw/Detroit_nbhoods/Master_Plan_Neighborhoods_Detroit.shp")

#Detroit.nhds <- Detroit.neighborhoods["NHOOD"]

#st_crs(Pb1981to2018.tidy)
#st_crs(Detroit.neighborhoods)
#mapview(Pb1981to2018.tidy["Site.Name"])
#mapview(Detroit.nhds)
```


###Most of the wrangling was done earlier in this document in order to show a tidy usable dataset for the kable table. But there are examples of the wrangling in the datasets below.
...
```{r 3.3 NOT FINISHED YET wrangle the .shp to make it usable}
...
print(Detroit.1980smap)
 #this is just a weird grid 
...
#FROM LESSON correct order to see mult datasets
ggplot() +
...
to see mapview of all sample sites: http://localhost:20087/session/viewhtml9a8d31b5fda/index.html
```{r 5.3 Map of sample sites graph, echo=FALSE}

# add locations to sf map, make larger circles for higher Pb concn, add labels to sites
summarise(Detroit.nhds)
st_bbox(Detroit.nhds)
mapview(Detroit.nhds)
Pb1981.tidy.sf <- st_as_sf(Pb1981.tidy) 
Detroit.nhds.sf <- st_as_sf(Detroit.nhds)

aqs_sites <- read.csv("EPA_Pb_data_Raw/aqs_sites.csv")
aqs_sites.geo <- st_as_sf(aqs_sites,
                          coords = c("Latitude", "Longitude"),
                          crs = 4326)
class(aqs_sites$Longitude)

aqs_sites2 <- mapview(aqs_sites, xcol = "Latitude", ycol = "Longitude",
        crs = 4326, grid = FALSE)

nhds.1981.join <- 

Detroit <- ggplot() +
  geom_sf(data = Pb1981.tidy.sf, aes(fill = geometry), color = "red") +
  geom_sf(data = aqs_sites, color = "green", size = 1) +
  scale_fill_gradient(low="yellow", high="darkgreen")
print(Detroit) #Error in is.finite(x) : default method not implemented for type 'list' <- Detroit.nhds is just names and a list
#prob need to create a join 



Detroit2 <- mapview(Pb1981to2018.gathered,
                    xcol = "SITE_LONGITUDE",
                    ycol = "SITE_LATITUDE",
                    crs = 4326,
                    grid = FALSE)
Detroit2 #http://localhost:20087/session/viewhtml9a8d31b5fda/index.html
