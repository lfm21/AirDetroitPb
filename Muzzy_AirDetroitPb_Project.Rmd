...
Format date to Date class for gathered data frame. 
Convert coordinates into geometry (using crs = 4326 from http://gis-michigan.opendata.arcgis.com/datasets/D3::master-plan-neighborhoods-detroit). 
...
```{r 1982 and 2018 - I PROB DONT NEED wrangling example datasets}
...
```{r 3.2 ATTEMPT to fix Site Names}
Pb1981to2018.names <- Pb1981to2018.tidy
```

```{r wrangle the shp to make it usable}
Detroit.neighborhoods <- 
  st_read("./Master_Plan_Neighborhoods_Detroit/Master_Plan_Neighborhoods_Detroit.shp")
```
