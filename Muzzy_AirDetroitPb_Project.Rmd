...
getwd()
``` 
The function rbind was used to combine all 37 datasets.
```{r read files, message=FALSE, warning=FALSE, include=FALSE, paged.print=FALSE}
...
Pb1981to2018.points <- list.files("./EPA_Pb_data_Processed/EPb1981to2018.points.csv", full.names = TRUE)

#Pb1981to2018.points <- read.csv("./EPA_Pb_data_Processed/EPb1981to2018.points.csv")

PbDetroit <- head(Pb1981to2018.points)
...
I chose to test the wrangling on the 1981 dataset. Then I converted the coordinate columns to geometry points so they can be plotted on a map; and added a month and year column in order to see if there's seasonal change.
```{r 3.1 - wrangling example datasets}
#as.Date(Pb1981$Date, format = "%m/%d/%Y") #takes up too much space

Pb1981.month.yr <- Pb1981 %>% dplyr::select( Date, Site.ID, Site.Name,
                                             Daily.Mean.Pb.Concentration, UNITS,
                                             SITE_LATITUDE, SITE_LONGITUDE) %>% 
  mutate(year = year(Date), month = month(Date)) 

#Pb1981.month.yr <- select(Pb1981, Date, Site.ID, Site.Name,
                          #Daily.Mean.Pb.Concentration, UNITS,
                          #SITE_LATITUDE, SITE_LONGITUDE)  %>% 
  #mutate(year = year(Date), month = month(Date)) 
...
Kruskal-Wallis tests performed on the whole large gathered dataset (Pb1981to2018.points), and on the dataset with the two largest sample sizes (Pb.large.sets). We see that there is a significant difference between the groups.
...
Pb1981to2018.kw
Pb1981to2018.wilcox <- 
  wilcox.test(Pb1981to2018.points$Daily.Mean.Pb.Concentration, mu = 50, alternative = "less")
Pb1981to2018.wilcox
...
Pb.large.sets.wilcox <- 
  wilcox.test(Pb.large.sets$Daily.Mean.Pb.Concentration, mu = 50, alternative = "less")
Pb.large.sets.wilcox
