...
There are a few variables that have a Site.Id but no Site.Name, so we fill the NAs with something slightly more descriptive.
...
```{r TEST --DUMP LATER-- JOIN AND MAPVIEW FOR .shp 1981 }
Pb1981.tidy <- Pb1981 %>% 
  select(Date, Site.ID, Daily.Mean.Pb.Concentration,
         UNITS, Site.Name, SITE_LATITUDE, SITE_LONGITUDE) %>% 
  mutate(year = year(Date), month = month(Date)) %>% 
...
Pb1981site <-  Detroit.nhds %>% left_join(y = Pb1981.tidy, by = c())
plot(Pb1981site)
             
```


```{r 3.3 wrangle the .shp to make it usable}
...
#plot(Detroit.1980smap) #this is just a weird grid 
...
```{r 5.1 NOT DONE Seasonality graph, echo=FALSE}
...
```{r 5.2 FIX COLORS child risk graph , echo=FALSE}
...
Pb.childrisk.plot2 <- ggplot(Pb.childrisk,
  ...
  geom_histogram(aes(), bins = 300, size = 1.5) +
  scale_fill_viridis_c() +
  scale_y_log10() +
...
Detroit <- ggplot() +
  geom_sf(data = Detroit.nhds, color = "green", size = 1) +
  geom_sf(data = Pb1981.tidy, aes(fill = geometry), color = "red") +
  scale_fill_gradient(low="yellow", high="darkgreen")
